<h2><?php echo $html->link($protected_section['ProtectedSection']['title'],"/protected_sections/view/{$protected_section['ProtectedSection']['id']}")?>: Manage Inline Media</h2>
<p>Return to .</p>
...
?><li><?php echo $this->renderElement('inline_media',array('media_data'=>$media_item,'parent'=>$inline_data,'media_previous'=>$previous_id))?></li><?php
$previous_id = $media_item['id'];
endforeach;?>
</ul>
<?php endif;?>
<?php if($inline_data['db_count']>$inline_data['actual_count']):?>
<form action="<?php echo $html->url("/".Inflector::underscore($this->name)."/edit/{$protected_section['ProtectedSection']['id']}")?>" method="post" enctype="multipart/form-data">
