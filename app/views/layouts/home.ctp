...
<?php if(!empty($metadata_for_layout)) echo $this->element('metadata',array('cache'=>true,'key'=>md5(serialize($metadata_for_layout)))) ?> 
<?php echo $this->element('css',array( 'cache'=>false,'key'=>'fcss')) ?> 
<?php echo $this->element('js',array( 'cache'=>false,'key'=>'fjs')) ?> 
</head>
<body id="home"><?php echo $session->check('Message.flash')?$session->flash():'' ?>
