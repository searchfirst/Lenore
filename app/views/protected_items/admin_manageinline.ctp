<h2><?php echo $html->link($protectedItem['ProtectedSection']['title'],"/protected_sections/view/{$protectedItem['ProtectedSection']['id']}")?> — <?php echo $html->link($protectedItem['ProtectedItem']['title'],"/protected_items/view/{$protectedItem['ProtectedItem']['id']}")?>: Manage Inline Media</h2>
...
<?php endif;?>
<?php if($inline_data['db_count']>$inline_data['actual_count']):?>
<form action="<?php echo $html->url("/".Inflector::underscore($this->name)."/edit/{$protectedItem['ProtectedItem']['id']}")?>" method="post" enctype="multipart/form-data">
