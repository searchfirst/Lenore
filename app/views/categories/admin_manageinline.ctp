<h2><?php echo $html->link($category['Category']['title'],"/categories/view/{$category['Category']['id']}") ?>: Manage Inline Media</h2>
...
?></ul><?php endif;?>
<?php if($inline_data['db_count']>$inline_data['actual_count']):?>
<form action="<?php echo $html->url("/".Inflector::underscore($this->name)."/edit/{$category['Category']['id']}")?>" method="post" enctype="multipart/form-data">
